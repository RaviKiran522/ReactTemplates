...
export default function CommonList(props: any) {
  ...
  const handleMenuOpen = (event: any, each: any) => {
        ...
        <MainCard style={{ borderColor: '#666666' }}>
          <Grid container sx={{ display: 'flex', flexDirection: 'row', padding: '10px 0px', marginBottom: '1' }}>
            ...
            <Grid item xs={12} display="flex" justifyContent="flex-end" padding="10px">
              <PaginationButtons count={Math.ceil(data.length / rowsPerPage)} page={pageNumber} onChange={handlePageChange} />
            </Grid>
          </Grid>
                ...
                <Grid container spacing={2}>
                    ...
                    <Card style={{ borderColor: '#4680FF', padding: '10px' }}>
                      <div style={{ textAlign: 'center' }}>
                        ...
                        <Typography variant="h6" color="error">
                          {each?.personalDetails?.fullname ? each.personalDetails.fullname : ''}
                          {each?.personalDetails?.surname ? each.personalDetails.surname : ''}
                        </Typography>
                        <Card style={{ marginTop: '2px', padding: '2px', borderColor: 'black' }}>AM101060</Card>
                  ...
                  <Grid item xs={12} sm={5} md={4} xl={4}>
                    <Card style={{ borderColor: '#4680FF', padding: '10px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <Typography>
                          {' '}
                          {each?.personalDetails?.dateOfBirth ? moment(each.personalDetails.dateOfBirth).format('DD/MM/YYYY') : ''}
                        </Typography>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <Grid item xs={12} sm={5} md={4} xl={2}>
                    <Card style={{ borderColor: '#4680FF', padding: '10px' }}>
                      {/* Actions Button */}
                      <div style={{ textAlign: 'center' }}>
                        <Button variant="contained" color="primary" onClick={(e: any)=>handleMenuOpen(e, each)}>
                          Actions
                        </Button>
                      </div>
                      ...
                      <Menu anchorEl={menuAnchorEl} open={Boolean(menuAnchorEl)} onClose={handleMenuClose}>
                        {actions.map((action: any, index: any) => (
                          <MenuItem key={index} onClick={() => handleActionSelect(action.id, selectedObject)}>
