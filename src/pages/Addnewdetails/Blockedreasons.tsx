...
export default function Blockedreasons() {
  ...
  //   debounce(() => blockedLists(), 500), // Adjust debounce time as needed
  //   []
  // );
  ...
  const handleFormSubmit = async () => {
    if (validate()) {
      const newRecord = {
        name: formData.blockedreasonsName.value,
        status: formData.statusName.value.label === "ENABLE" ? 1 : 0,
      };
      setIsLoading(true);
      const result = await createBlockedReasons(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
        ...
        setIsLoading(false);
        await blockedLists(); // Explicitly call here
        setTimeout(() => {
          setOpenPopup(false);
  ...
  const columns = useMemo(
    () => [
      ...
      { header: "Blocked Reasons Name", accessorKey: "blockedreasons" },
      {
        header: "Status",
        accessorKey: "status",
  ...
  const handleEditFormSubmit = async () => {
    if (validate()) {
      ...
      setIsLoading(true);
      const result = await editBlockedReasons(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
          message: result.message,
  ...
  const blockedLists = async () => {
    setListLoader(true);
    const result = await listBlockedReasons(listFilter);
    if (result.status) {
      setListLoader(false);
      setRowCount(result.totalCount);
      if (result.data.length > 0) {
        const data = result.data.map((item: any, index: any) => ({ id: item.id, sno: listFilter.skip + index + 1, blockedreasons: item.name, status: item.status ? 'Enable' : 'Disable' }));
        setTableData(data);
      }
          ...
          {selectedRow && (
            <Grid textAlign={'center'}>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
                <strong>Blocked Reasons Name:</strong> {selectedRow.blockedreasons}
              </Typography>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
