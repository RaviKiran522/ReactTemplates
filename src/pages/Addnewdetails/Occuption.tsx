...
export default function Occuption() {
  ...
  const handleFormSubmit = async () => {
    if (validate()) {
      const newRecord = {
        name: formData.occuptionName.value,
        status: formData.statusName.value.label === "ENABLE" ? 1 : 0,
      };
      setIsLoading(true);
      const result = await createOccuption(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
        ...
        setIsLoading(false);
        await occuptionList(); // Explicitly call here
        setTimeout(() => {
          setOpenPopup(false);
  ...
  const occuptionList = async () => {
    setListLoader(true);
    const result = await listOccuption(listFilter);
    if (result.status) {
      setListLoader(false);
  ...
  //   debounce(() => occuptionList(), 500), // Adjust debounce time as needed
  //   []
  // );
  ...
  const initailData: any = [
    ...
    { sno: "3", occuption: "AKNU", status: "Enable" },
    { sno: "4", occuption: "BOOKS", status: "Disable" },
    { sno: "5", occuption: "MUSIC", status: "Enable" },
    { sno: "6", occuption: "SELF", status: "Enable" },
    { sno: "7", occuption: "TV", status: "Enable" },
    { sno: "8", occuption: "MOBILE", status: "Enable" },
    { sno: "9", occuption: "LAPTOP", status: "Enable" }
  ];
  ...
  const columns = useMemo(
    () => [
      ...
      { header: "Occuption Name", accessorKey: "occuption" },
      {
        header: "Status",
        accessorKey: "status",
  ...
  const handleEdit = (row: any, action: any) => {
    ...
    newFormData.occuptionName.value = row.occuption;
  ...
  const handleEditFormSubmit = async () => {
    if (validate()) {
      ...
      setIsLoading(true);
      const result = await editOccuption(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
          message: result.message,
  ...
  return (
    <>
      ...
      <Dialog open={openPopup} maxWidth="sm" fullWidth>
        ...
        <DialogTitle> Create Occuption</DialogTitle>
        <DialogContent>

          <Grid item xs={12} padding={2}>
            <CommonInputField inputProps={formData.occuptionName} onChange={handleChange} />
          </Grid>
          ...
          {selectedRow && (
            <Grid  textAlign={'center'}>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
                <strong>Occuption Name:</strong> {selectedRow.occuption}
              </Typography>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
