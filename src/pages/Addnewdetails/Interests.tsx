...
export default function Interests() {
  ...
  //   debounce(() => intersetsList(), 500), // Adjust debounce time as needed
  //   []
  // );
  ...
  const handleFormSubmit = async () => {
    if (validate()) {
      const newRecord = {
        name: formData.interestsName.value,
        status: formData.statusName.value.label === "ENABLE" ? 1 : 0,
      };
      setIsLoading(true);
      const result = await createInterests(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
        ...
        setIsLoading(false);
        await intersetsList(); // Explicitly call here
        setTimeout(() => {
          setOpenPopup(false);
  ...
  const handleEdit = (row: any, action: any) => {
    ...
    const newFormData = _.cloneDeep(formData);
    newFormData.interestsName.value = row.interests;
  ...
  const handleEditFormSubmit = async () => {
    if (validate()) {
      ...
      setIsLoading(true);
      const result = await editinterests(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
          message: result.message,
  ...
  const intersetsList = async () => {
    setListLoader(true);
    const result = await listInterests(listFilter);
    if (result.status) {
      setListLoader(false);
      setRowCount(result.totalCount);
      if (result.data.length > 0) {
        const data = result.data.map((item: any, index: any) => ({ id: item.id, sno: listFilter.skip + index + 1, interests: item.name, status: item.status ? 'Enable' : 'Disable' }));
        setTableData(data);
      }
          ...
          {selectedRow && (
            <Grid textAlign={'center'}>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
                <strong>Interests Name:</strong> {selectedRow.interests}
              </Typography>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
