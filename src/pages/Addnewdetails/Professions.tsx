...
import { createProfession, editProfession, listProfession, professionStatus } from 'services/add-new-details/AddNewDetails';
...
export default function Professions() {
  ...
  const handleFormSubmit = async () => {
    if (validate()) {
      const newRecord = {
        name: formData.professionName.value,
        status: formData.statusName.value.label === "ENABLE" ? 1 : 0,
      };
      setIsLoading(true);
      const result = await createProfession(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
        ...
        setIsLoading(false);
        await professionList(); // Explicitly call here
        setTimeout(() => {
          setOpenPopup(false);
  ...
  const updateProfessionHandler = async (updateData: any = {}, multiple = "") => {
      setIsLoading(true);
      if(!multiple) {
        let d = Object.keys(updateData).length;
        const updateRecord = {
          name: d> 0 ? updateData?.profession : formData.professionName.value,
          status: d > 0 ? (updateData?.status === "Enable" ? 0 : 1) : (formData.statusName.value.label === 'ENABLE' ? 1 : 0) ,
          id: d > 0 ? updateData.id : openPopup
        }
        const update = await professionStatus(updateRecord);
        if (update.status) {
        ...
        updateResult = await professionStatus(payload);
  ...
  const columns = useMemo(
    () => [
      ...
      { header: "Professions Name", accessorKey: "profession" },
      {
        header: "Status",
        accessorKey: "status",
  ...
  const handleEdit = (row: any, action: any) => {
    ...
    const newFormData = _.cloneDeep(formData);
    newFormData.professionName.value = row.profession;
  ...
  const buttonHandler = (action: string, users: any) => {
    ...
    if(action === "disable") {
      updateProfessionHandler(users);
    } else if(action === "ENABLE") {
      updateProfessionHandler(users, "ENABLE");
    } else if(action === "DISABLE") {
      updateProfessionHandler(users, "DISABLE");
    }
  }
  ...
  const handleEditFormSubmit = async () => {
    if (validate()) {
      ...
      setIsLoading(true);
      const result = await editProfession(newRecord);
      if (result.status) {
        setSuccessBanner({
          flag: true,
          message: result.message,
  ...
  const professionList = async () => {
    setListLoader(true);
    const result = await listProfession(listFilter);
    if (result.status) {
      setListLoader(false);
      setRowCount(result.totalCount);
      if (result.data.length > 0) {
        const data = result.data.map((item: any, index: any) => ({ id: item.id, sno: listFilter.skip + index + 1, profession: item.professionName, status: item.status ? 'Enable' : 'Disable' }));
        setTableData(data);
      }
      ...
      <Dialog open={openPopup} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Grid item xs={12} padding={2}>
            <CommonInputField inputProps={formData.professionName} onChange={handleChange} />
          </Grid>
          <Grid item xs={12} padding={2} >
          ...
          {selectedRow && (
            <Grid  textAlign={'center'}>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
                <strong>Professions Name:</strong> {selectedRow.profession}
              </Typography>
              <Typography sx={{ fontWeight: 400, fontSize: '1rem', marginBottom: '5px' }}>
